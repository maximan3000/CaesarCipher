...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        encryption = CaesarFabric.getEncryptionSystem();
    ...
    private static void executeHack(String encryptedFile) {
        try {
            String encryptedText = sourceFiles.getFileContent(encryptedFile);
            Hack hack = HackFactory.getCaesarEncryptionHack(encryptedText, sourceOfDictionary);
    ...
    static void handHack(Hack hack, String hackedFile) throws Exception {
        executeHackHelp();
        while (Boolean.TRUE){
            String command = scanner.nextLine();
            try {
    ...
    private static void parseHackCommand(String command, Hack hack, String hackedFile) throws Exception {
        command = command.trim();
        String[] commandData = command.split(" ");
        String commandName = commandData[0];
    ...
    private static void executeHackSwap(String from, String to, Hack hack) {
        try {
    ...
    private static void executeHackApply(Hack hack, String hackedFile) {
        try {
            String hackedText = hack.hack();
            sourceFiles.putNewFileContent(hackedFile, hackedText);
            System.out.println("Hacked text moved to file: " + hackedFile + "\n");
